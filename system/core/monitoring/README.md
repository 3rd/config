...
- a long-lived audit stream exporter that backfills once, then consumes `audisp-af_unix` events live
...
- the audit pipeline is stream-driven after startup backfill; only network-usage remains timer-driven by default.
